...
def load_trace_data(n=None, session_id=session_id):
    ...
    data_path = f'../azure_2019/vmtable.csv.gz'
...
# def create_coassociation_matrix_v1(labels):
#     """
#     Create a co-association matrix from the clustering result labels.
#     Co-association matrix is a symmetric matrix where each entry co[i,j] signifies
#     how many times points i and j were in the same cluster.
#
#     :param labels: 2D list, with each inner list representing the clustering labels
#                    from a different clustering algorithm.
#     :returns: DataFrame where each entry [i,j] is the co-association between
#               points i and j.
#     """
#     n_samples = len(labels[0])
#     coassoc_matrix = pd.DataFrame(np.zeros((n_samples, n_samples)))
#
#     for label in labels:
#         for cluster_id in np.unique(label):
#             indices = np.where(label == cluster_id)[0]
#             for index1 in indices:
#                 for index2 in indices:
#                     coassoc_matrix.loc[index1, index2] += 1
#                     if index1 != index2:
#                         coassoc_matrix.loc[index2, index1] += 1
#
#     # Normalize co-association matrix
#     num_clustering = len(labels)
#     coassoc_matrix /= num_clustering
#
...
def create_coassociation_matrix(labels):
    ...
    :param labels: 2D list, with each inner list representing the clustering labels
                   from a different clustering algorithm.
    :returns: Sparse DataFrame where each entry [i,j] is the co-association between
              points i and j.
    """
    ...
    # Create a sparse matrix for co-association
    coassoc_matrix = dok_matrix((n_samples, n_samples), dtype=np.float)
    ...
    for label in labels:
        for cluster_id in np.unique(label):
            ...
            for index1 in indices:
                for index2 in indices:
                    coassoc_matrix[index1, index2] += 1
                    if index1 != index2:
                        coassoc_matrix[index2, index1] += 1

    # Normalize co-association matrix
    num_clustering = len(labels)
    coassoc_matrix /= num_clustering

    # Convert to COO for efficient computation
    coassoc_matrix_coo = coo_matrix(coassoc_matrix)

    # Convert sparse matrix to Sparse DataFrame for easier manipulation
    coassoc_df = pd.DataFrame.sparse.from_spmatrix(coassoc_matrix_coo)
...
def main():
    ...
    for l in Ensemble_Cluster_cols:
        ...
        umap = UMAPVisualizer(random_state=session_id, metric='cosine',
                              title=f"UMAP projection (via SP, {l.split('_')[-2]}, {l.split('_')[-1]})",
                              size=(650, 380),
                              fontdict={'fontsize': 600})
        umap.fit(SP.X_transformed, trace_dataframe_ensemble_clustered_f[l])
        umap.show(outpath=f"./out_figs/SP_{l.split('_')[-2]}_{l.split('_')[-1]}_umap_cosine.png",
                  clear_figure=True)

        # tsne = TSNEVisualizer(random_state=session_id, decompose_by=20)
        # tsne.fit(SP.X_transformed, trace_dataframe_ensemble_clustered_f[Ensemble_Cluster_cols[3]].to_list())
        # tsne.show(outpath=f'./out_figs/SP_PCA_kmeans_KElbow_distortion.png',
        #           clear_figure=True)
